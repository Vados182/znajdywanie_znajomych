...
# Poprawiona, dwuwymiarowa macierz środków klastrów wyciągnięta z Twojego modelu
CENTROIDS = np.array([
...
def build_features_vector(row):
    """Przekształca słownik lub wiersz DataFrame na rzadki wektor One-Hot Encoding (0 i 1)"""
    vector = []
...
def predict_closest_cluster(row, centroids_matrix):
    """Wylicza odległość euklidesową do każdego środka klastra i zwraca najbliższy index"""
    features_vector = build_features_vector(row)
    
    # Zabezpieczenie dopasowania rozmiaru macierzy, jeśli liczby kolumn minimalnie się różnią
